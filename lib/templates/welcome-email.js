...
module.exports = (event, callback) => {
  ...
  const emailBody = encodeURIComponent(`
    <p>Fellow ${memberAliasFor(event.group)},</p>
    ...
    <p>${firstNameFor(organizer)}</p>
  `).replace(/>\s+</g, '><')

  const welcomeTemplateLink = `https://www.meetup.com/${event.group}/messages/send/?who=oneevent&eventId=${event.id}&boards=1&subject=${emailSubject}&body=${emailBody}`
