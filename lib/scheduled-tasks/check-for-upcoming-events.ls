  ...
  new UpcomingEventsFetcher(robot).all (events) !->
    ...
    for event in events
      ...
      if is-today event
        ...
        robot.message-room announcement-room, "WAHH! Meetup tonight! It's \"#{event.attributes.name}\" at #{format-time-only event.attributes.time.absolute}. Learn more and RSVP at #{event.links.self}"
