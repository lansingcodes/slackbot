...
describe('check-for-upcoming-events', () => {
  ...
  describe('when there is at least 1 new event', () => {
    ...
    const checkForUpcomingEvents = proxyquire(
      '../../../lib/scheduled-tasks/check-for-upcoming-events',
      {
      ...
      robot.adapter.on('send', (envelope, strings) => {
        ...
        if (messageCount === 1) {
          ...
          expect(strings[0]).toMatch(
            /Good morning everyone! Yawn\.\.\. I'm gonna grab .+ and check for newly scheduled meetups\./
          )
          done()
        }
      })
      ...
      robot.adapter.on('send', (envelope, strings) => {
        ...
        if (messageCount === 2) {
          ...
          expect(strings[0]).toEqual(
            "There's a new event scheduled: <meetup.com|Today's new JavaScript framework>. Follow the link to learn more and RSVP."
          )
          done()
        }
      })
